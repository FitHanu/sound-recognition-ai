...
class ThaoLp(DataSet):
    ...
    def __init__(self):
        ...
        self.ds_abs_path = os.path.join(cache_dir, "datasets", key)
        # Ensure the directory exists before calling super()
        os.makedirs(self.ds_abs_path, exist_ok=True)
        super().__init__(key)

    def download(self):
        zip_url = "https://drive.google.com/uc?id=1YQrV_45hnL4EgXwiU9Yqu1HTCe-RkMFD"
        zip_path = os.path.join(self.ds_abs_path, "Sound.zip")
        extract_path = os.path.join(self.ds_abs_path, "ThaoLp")

        # Ensure the directory exists before downloading the file
        os.makedirs(self.ds_abs_path, exist_ok=True)

        gdown.download(zip_url, "Sound.zip", quiet=False)
        extract("Sound.zip", extract_path)
        return extract_path 
